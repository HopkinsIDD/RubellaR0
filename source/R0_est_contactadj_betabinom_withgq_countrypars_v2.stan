...
data {
  ...
  array[S] int<lower=1> country_of_survey;        // Country id for each survey
...
parameters {
  ...
  real<lower=0,upper=3> sigma_g;             // Standard deviation of global R0 distribution
  vector<lower=0,upper=3>[C] sigma_c;        // Standard deviation of country R0 distribution
...
model { 
  ...
  target += normal_lpdf(logR0_s | logR0_c[country_of_survey], sigma_c[country_of_survey]); // Centered version
