...
analysis_version_data <- "July2024"
analysis_version <- "July2024"
analysis_version_repo <- file.path( "results", analysis_version)
analysis_version_data_repo <- file.path( "data", analysis_version_data)
dir.create(analysis_version_data_repo, recursive = TRUE)
dir.create(analysis_version_repo, recursive = TRUE)
countries_of_interest_path <- "data/vimc_countries_of_interest_202110gavi_v1.csv"
...
save.data <- TRUE
sero_update_files <- list.files("data/July2024/raw_data", full.names = TRUE, pattern = ".csv") # c("sero_2021update.csv", "sero_2024May_update.csv", "sero_2024July_update.csv")
pull_raw_data <- TRUE
...
#source("source/load_and_clean_R0_data.R")
...
# because WPP is sooooo slow, we will extract those data here first and resave as parquets for easy querying
...
# Prepared in 
# source("source/preprocessing/setup_R0_est_data_stan.R")

# Load needed data

if (load_saved_data){
  load(file=file.path(analysis_version_data_repo, "R0_est_testing_AGE_DATA.RData")) # Loads sero_data_stan, sero_data
} else {
  
  #load(file='data/sero_data_stan.Rdata') # Loads 'sero_data_stan'
  #load(file="data/sero_data.RData")      # Loads 'sero_data'
  sero_data_name <- file.path("data", analysis_version, "sero_data_final.csv")
  sero_data <- read_csv(sero_data_name) %>% as_tibble()
  
  # Remove unneeded columns
  cols_to_remove <- c("further_review_needed", "check_notes", "dupl_othertest", "manual_skip", "entered_by", "URL",
                      "second_reviewer, -Checked", "X30", "X31", "author", "unique_ref_entry_withID")
  cols_to_remove <- cols_to_remove[cols_to_remove %in% colnames(sero_data)]
  sero_data <- sero_data %>% dplyr::select(-any_of(cols_to_remove))
  ...
  save(sero_data, sero_data_stan, countryregion_of_survey, region_of_country,   file=file.path(analysis_version_data_repo, "R0_est_testing_AGE_DATA.RData")) # Loads sero_data_stan, sero_data
...
# Add r_ratio to stan list
source("source/get_ageadj_R0.R")

country_year <- read.csv(file=file.path("data", analysis_version_data, "r_ratio_1980.csv"))
tmp_data <- sero_data %>% 
  filter(survey.ind %in% sero_data_stan$survey_ind) %>%
  select(survey.ind, ISO, year=start.year) %>%
  distinct() %>%
  left_join(country_year %>% select(ISO, year, r_ratio_1980))

sero_data_stan$r_ratio <- tmp_data$r_ratio_1980
...
# ----> Figure this out. do we need a plus 1???
...
# Using 0 is likely most correct as we already adjust for the population structure somewhat.
...
# save.image(file.path("data",analysis_version,"R0_stan_sero.Rdata"))
# load(file.path("data",analysis_version,"R0_stan_sero.Rdata"))
...
source("source/plot_serosurvey_curves.R")
...
sum(sero_data$max_age_years<sero_data$min_age_years)
...
#**** Save final data ****
save(sero_data, sero_data_stan, countryregion_of_survey, region_of_country, file=file.path(analysis_version_data_repo, "R0_est_final_data.RData")) # Loads sero_data_stan, sero_data
...
# load data
# - Loads sero_data, sero_data_stan, countryregion_of_survey, region_of_country
load(file=file.path(analysis_version_data_repo, "R0_est_final_data.RData")) 


#fileName_R0_est <- "source/R0_est_fit_sero.stan"
fileName_R0_est <- "source/R0_est_contactadj_betabinom_withgq.stan"
fileName_R0_est <- "source/R0_est_contactadj_betabinom_withgq_countrypars.stan"

ret <- stanc(fileName_R0_est) # Check Stan file
ret_sm <- stan_model(stanc_ret = stanc(fileName_R0_est)) # Compile Stan code
save(fileName_R0_est, ret, ret_sm, file="source/R0_est_contactadj_betabinom_withgq_countrypars_compiled.RData")
#load(file="source/R0_est_contactadj_betabinom_withgq_compiled.RData")
load(file="source/R0_est_contactadj_betabinom_withgq_countrypars_compiled.RData")


# fileName_R0_est <- "source/R0_est_original_betabinom.stan"
# ret <- stanc(fileName_R0_est) # Check Stan file
# ret_sm <- stan_model(stanc_ret = stanc(fileName_R0_est)) # Compile Stan code
# save(fileName_R0_est, ret, ret_sm, file="source/R0_est_original_betabinom_compiled.RData")
# load(file="source/R0_est_original_betabinom_compiled.RData")
...
inits_ <- list(b0=rep(0.3, n_survey),
               b1=rep(0.6, n_survey),
               b2=rep(0.03, n_survey),
               logR0_g = log(6.0), 
               logR0_c = rep(log(6.0),sero_data_stan$C), 
               logR0_s = rep(log(6.0),sero_data_stan$S),
               #logR0_s_raw = rep(0.1,sero_data_stan$S),
               #logR0_c = rep(log(5.0),sero_data_stan$C),
               #logR0_c_raw = rep(0.1,sero_data_stan$C),
               sigma_g = 0.05, 
               sigma_c = 0.05,
               #sigma_c = rep(0.05,sero_data_stan$C),
               gamma = .035,
...
fit_R0 <- rstan::stan(#model_code = ret_sm,
  file = fileName_R0_est, 
  chains = 4,
  iter = 200, 
  data = sero_data_stan,
  init = inits)

# fit_R0 <- sampling(ret_sm, warmup=100, iter=200, seed=12345, data=sero_data_stan, 
...
# Save STAN results
#save(ret_sm, fit_R0_test, sero_data_stan, sero_data, file="results/R0_full_test_500iters.RData")
save(ret_sm, fit_R0, sero_data_stan, sero_data, file=file.path(analysis_version_repo, "R0_full_test_200iters.RData"))
load(file=file.path(analysis_version_repo, "R0_full_test_200iters.RData")) # Loads ret_sm, fit_R0_test, sero_data_stan_list



# Traceplot
rstan::traceplot(fit_R0, inc_warmup = T)
tmp <- rethinking::precis(fit_R0, depth = 2)
head(tmp)
...
R0_s_adj <- fits$R0_s_adj
exp(colMeans(log(R0_s_adj)))
...
logR0_c <- fits$logR0_c
exp(colMeans(logR0_c))
summary(colMeans(logR0_c))
summary((as.numeric(logR0_c)))
...
b0 <- apply(fits$b0, 2, median); summary(b0)
b1 <- apply(fits$b1, 2, median); summary(b1)
b2 <- apply(fits$b2, 2, median); summary(b2)
b_fits <- data.frame(survey_num = sero_data_stan$survey[!duplicated(sero_data_stan$survey)], b0=b0, b1=b1, b2=b2 )

source("source/plot_serosurvey_curves.R")
plot_serosurvey_curves_withfits(stanfits = file.path(analysis_version_repo, "R0_full_test_200iters.RData"), proj_dir=analysis_version_repo)
...
# Traceplot
rstan::traceplot(fit_R0, inc_warmup = T)
ests <- rethinking::precis(fit_R0, depth = 2, prob=0.5) 
ests95 <- rethinking::precis(fit_R0, depth = 2, prob=0.95)

ests <- ests %>% mutate(param = rownames(ests)) %>%  as_tibble() %>%
  separate(param, c("param2","ind"), sep="[|]", remove=FALSE)

# ~ Examine Results ---------------------------------------------------------

fits <- rstan::extract(fit_R0, inc_warmup=FALSE)
...
source("source/plot_serosurvey_curves.R")
plot_serosurvey_curves_withfits(stanfits = file.path(analysis_version_repo, "R0_full_3000iters.RData"), proj_dir=analysis_version_repo)
...
R0c_ests <- ests[grepl("R0c", rownames(ests)),]
R0c_ests <- R0c_ests %>%
  mutate(country_num = rownames(R0c_ests)) %>%
  mutate(country_num = gsub("R0c\\[", "", country_num),
         country_num = as.integer(gsub("]", "", country_num)))
R0c_ests <- R0c_ests %>% mutate(ISO = countryregion_of_survey$ISO[match(country_num, sero_data_stan$country_of_survey)])


logR0c_ests <- ests[grepl("logR0_c", rownames(ests)),]
logR0c_ests <- logR0c_ests %>%
  mutate(country_num = rownames( logR0c_ests)) %>%
  mutate(country_num = gsub("logR0_c\\[", "", country_num),
         country_num = as.integer(gsub("]", "", country_num)))
logR0c_ests <- logR0c_ests %>% mutate(ISO = countryregion_of_survey$ISO[match(country_num, sero_data_stan$country_of_survey)])

sigma_c_ests <- ests[grepl("sigma_c", rownames(ests)),]
sigma_c_ests <- sigma_c_ests %>%
  mutate(country_num = rownames( sigma_c_ests)) %>%
  mutate(country_num = gsub("sigma_c\\[", "", country_num),
         country_num = as.integer(gsub("]", "", country_num)))
sigma_c_ests <- sigma_c_ests %>% mutate(ISO = countryregion_of_survey$ISO[match(country_num, sero_data_stan$country_of_survey)])


logR0g_ests <- ests[grepl("logR0_g", rownames(ests)),]
logR0g_ests <- logR0g_ests %>%
  mutate(country_num = rownames( logR0g_ests)) %>%
  mutate(country_num = gsub("logR0_g\\[", "", country_num),
         country_num = as.integer(gsub("]", "", country_num)))
logR0g_ests <- logR0g_ests %>% mutate(ISO = countryregion_of_survey$ISO[match(country_num, sero_data_stan$country_of_survey)])


sigma_g_ests <- ests[grepl("sigma_g", rownames(ests)),]
sigma_g_ests <- sigma_g_ests %>%
  mutate(country_num = rownames( sigma_g_ests)) %>%
  mutate(country_num = gsub("sigma_g\\[", "", country_num),
         country_num = as.integer(gsub("]", "", country_num)))
sigma_g_ests <- sigma_g_ests %>% mutate(ISO = countryregion_of_survey$ISO[match(country_num, sero_data_stan$country_of_survey)])




# Check logR0 and R0 matching (since R0 from generated quantiles)
R0c_ests2 <- R0c_ests %>% select(ISO, R0 = mean, R0_sd = sd, R0_ll=`25%`, R0_ul=`75%`) %>%
  full_join(logR0c_ests %>% select(ISO, logR0 = mean, logR0_sd = sd, logR0_ll=`25%`, logR0_ul=`75%`) %>%
              mutate(R0_logversion = exp(logR0)))
i=2
test <- (exp(rnorm(10000, mean=R0c_ests2$logR0[i], sd=R0c_ests2$logR0_sd[i])))
hist(test, breaks=500)    
R0c_ests2[i,]
summary(test)
...
# R0_country_summary <- R0_country_summary %>%
#     mutate(ISO = country) %>%
#     left_join(R0c_ests, by=c("ISO"="ISO")) %>%
#     mutate(mean = mean.y, 
#            LL = `25%`,
#            UL = `75%`) %>%
#     select(country, ISO, mean, LL, UL)
# 
...
region_ <- get_region(country_)
subregion_ <- get_subregion(country_)

region_info <- data.frame(country=as.character(country_), region=region_, subregion=subregion_) %>%
  mutate(ISO = country) %>%
  full_join(R0_country_summary)
...
region_res <- region_res %>% distinct()
...
subregion_summ <- region_res %>% group_by(subregion) %>% 
  summarise(mean_logR0 = mean(logR0),
            low_logR0 = quantile(logR0, probs = 0.25),
            high_logR0 = quantile(logR0, probs= 0.75),
            logR0_sd = sd(logR0)) %>%
  mutate(meanR0 = exp(mean_logR0), 
         lowR0 = exp(low_logR0), 
         highR0 = exp(high_logR0))
...
priority_countries <- priority_countries %>% 
  mutate(Country = sapply(ISO, get_country_name_ISO3), sub.region=get_subregion(ISO)) %>% 
...
priority_countries <- left_join(priority_countries, R0_country_summary, by=c("ISO"="country")) %>% 
  rename(R0_mean=mean, R0_ll=LL, R0_ul=UL, logR0_mean=log_mean, logR0_sd=log_sd) %>% 
  #select(-median) %>%
...
# logR0g_sd <- sd(fits$logR0_g + fits$sigma_g)
# logR0g_sd <- mean(fits$sigma_g)
logR0g_sd <- median(fits$sigma_g)
...
priority_countries_previous <- read_csv(file.path("results/Jan2020_farrington_method","R0_distributions.csv"))

priority_countries <- read_csv(file.path(analysis_version_repo,"R0_distributions.csv"))


r0_data <- bind_rows(priority_countries %>% mutate(Version ="Oct 2021"),
                     priority_countries_previous %>% mutate(Version ="Jan 2020"))


r0_data <- r0_data %>% 
  mutate(continent = globaltoolboxlite::get_region(ISO)) %>%
  mutate(continent = ifelse(ISO=="XK", "Europe", continent))
...
cowplot::plot_grid(
  r0_data %>%
    ...
    ggplot() +
    geom_point(aes(x=ISO, y=R0_mean, group=ISO, color=Version), alpha=.6, size=2) +
    geom_errorbar(aes(x=ISO, ymin=R0_ll, ymax=R0_ul, group=ISO, color=Version), alpha=.5, size=1.2) +
  ...
  align = "h", axis="lr", nrow=2, rel_heights=c(2,1.1))
