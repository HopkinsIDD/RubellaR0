...
# Prepared in 
#source("source/setup_R0_est_data_stan.R")
...
if (load_saved_data){
    ...
    sero_data <- read_csv(file.path("data", analysis_version_data, "sero_data_final.csv")) %>% as.data.frame()
    sero_data_name <- file.path("data", analysis_version_data, "sero_data_final.csv")
...
fileName_R0_est <- "source/R0_est_contactadj_betabinom_withgq_countrypars_v2.stan"
...
rethinking::precis(fit_R0, depth = 2)
