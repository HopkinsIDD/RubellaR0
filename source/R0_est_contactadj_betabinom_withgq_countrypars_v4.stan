...
data {
  ...
  array[SA] int<lower=1> N;                       // Number of indivs in each age grp in each serosurvey/sample
  array[SA] int<lower=0> Y;                       // Number of seropositive in each age in each serosurvey/sample
  ...
  array[SA] int<lower=1> country;                 // Country id number for each survey/age
  array[SA] int<lower=1> survey;                  // Survey id number for each survey/age
  array[SA] int<lower=1> country_of_survey;        // Country id for each survey
...
parameters {
  ...
  real<lower=0,upper=1> b0;             // seroprev function parameter
  real<lower=.00001,upper=1> b1;        // seroprev function parameter
  real<lower=0,upper=0.05> b2;          // seroprev function parameter
  
  vector<lower=0,upper=1>[S] b0_s;             // seroprev function parameter
  vector<lower=.00001,upper=1>[S] b1_s;        // seroprev function parameter
  vector<lower=0,upper=0.05>[S] b2_s;          // seroprev function parameter
  vector<lower=0,upper=1>[S] b0_sigma_s;             // seroprev function parameter
  vector<lower=.00001,upper=1>[S] b1_sigma_s;        // seroprev function parameter
  vector<lower=0,upper=0.05>[S] b2_sigma_s;          // seroprev function parameter
...
transformed parameters {
  ...
  for (s in 1:S) {
    ...
    for (a in 1:age_pred_length){
      p_inf_given_age[a] = x_a_funct(age_pred[a]+1, b0_s[s], b1_s[s], b2_s[s]) - x_a_funct(age_pred[a], b0_s[s], b1_s[s], b2_s[s]); 
