...
//   real R0c[C];
//   
//   // Use the current estimates of b0, b1, b2 to generate R0_survey
//   for (s in 1:S) {
...
//     }
//     Ave_age_post[s] = sum((n_new_inf_age_post / sum(n_new_inf_age_post)) .* age_pred));
//     // R0_survey[s] = 1 + ( (1/mu[s]) / Ave_age_post[s] );     // R0
//   }
//   R0_survey = plus1 + 1 ./ ((Ave_age_post - M) .* mu);                          // R0 
//   
